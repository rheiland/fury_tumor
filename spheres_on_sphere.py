...
sphere_radius = np.sqrt(sphere_radius2)
inner_radius = sphere_radius - 2*cell_radius
inner_radius2 = inner_radius * inner_radius
print(f"sphere_radius2={sphere_radius2}, sphere_radius={sphere_radius}")
...
for z in np.arange(-box_radius, box_radius, z_spacing):
    ...
    for y in np.arange(-box_radius, box_radius, y_spacing):
        ...
        for x in np.arange(-box_radius, box_radius, x_spacing):
            ...
            # just keep hemisphere shell
            xyz_sq = (xsq + ysq + zsq)
            if (z<0.0) and (xyz_sq < sphere_radius2) and (xyz_sq > inner_radius2):  # assume centered about origin
